...
export default function Dashboard({ userId }: { userId: string }) {
  ...
  async function loadCreators() {
    ...
    setCreators(rows);
    if (!creatorId && rows.length) setCreatorId(rows[0].id);
  }
  ...
  async function loadWorkspace(id: string) {
    const [posts, recs] = await Promise.all([
      supabase
        .from("content_items")
        .select("id,platform,title,caption,views,link_clicks,revenue")
        .eq("creator_id", id)
        .order("created_at", { ascending: false }),
      supabase
        .from("recommendations")
        .select("id,title,summary,reason,goal,effort_minutes,status")
        .eq("creator_id", id)
        .order("created_at", { ascending: false }),
    ]);
  ...
  useEffect(() => { if (creatorId) loadWorkspace(creatorId); }, [creatorId]);
  ...
  async function createCreator(event: FormEvent<HTMLFormElement>) {
    ...
    await loadCreators();
    if (data?.id) setCreatorId(data.id);
  ...
  async function addContent(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("Content added. Add a few examples, then generate recommendations.");
  }

  async function generateRecommendations() {
    if (!creatorId || !activeCreator) return;
    const ranked = [...content].sort((a, b) => {
      const aScore = Number(a.revenue) * 100 + Number(a.link_clicks) * 4 + Number(a.views) / 1000;
      const bScore = Number(b.revenue) * 100 + Number(b.link_clicks) * 4 + Number(b.views) / 1000;
    ...
    const rows = winner ? [
      {
        creator_id: creatorId,
        title: `Follow up on ${winner.title || "your strongest post"}`,
        summary: "Reuse the winning idea with a different hook instead of starting from scratch.",
        reason: "It has the strongest blended signal from revenue, clicks and views in the content you entered.",
        goal: Number(winner.revenue) > 0 || Number(winner.link_clicks) > 0 ? "monetization" : "growth",
        confidence: 0.82,
        effort_minutes: 25,
      },
      {
        creator_id: creatorId,
        title: `Create a simple ${niche} recommendation post`,
        summary: "Teach, compare or recommend one useful thing your audience can act on.",
        reason: "This creates a clean test for profile intent and future affiliate opportunities.",
        goal: "learning",
        confidence: 0.68,
        effort_minutes: 20,
      },
      {
        creator_id: creatorId,
        title: "Test one new hook",
        summary: "Keep the subject familiar and change only the first few seconds.",
        reason: "Controlled variants help CreatorHub learn why something worked.",
        goal: "growth",
        confidence: 0.72,
        effort_minutes: 15,
      },
      ...
      {
        creator_id: creatorId,
        title: `Create your first ${niche} baseline post`,
        summary: "Start with one low-effort piece so CreatorHub has something to learn from.",
        reason: "There is not enough performance history yet for a personalized recommendation.",
        goal: "learning",
        confidence: 0.60,
        effort_minutes: 20,
      },
    ...
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: 24 }}>
      ...
      <section style={{ ...card, marginBottom: 16 }}>
        <select style={{ ...input, margin: 0 }} value={creatorId} onChange={(event) => setCreatorId(event.target.value)}>
          ...
          {creators.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.creator_type}</option>)}
        </select>
        {activeCreator && <p><a href={`/s/${activeCreator.slug}`} target="_blank">Open {activeCreator.name}&apos;s SmartLink →</a></p>}
      </section>

      {!activeCreator ? (
        <section style={card}><h2>Create your first workspace</h2><CreatorForm onSubmit={createCreator} /></section>
      ) : (
        <>
          ...
          <h2>Today</h2>
          <button style={primaryButton} onClick={generateRecommendations}>Generate recommendations</button>
          ...
          <form onSubmit={addContent} style={card}>
            <select name="platform" style={input}><option>TikTok</option><option>Instagram</option><option>Facebook</option><option>YouTube</option><option>X</option><option>Other</option></select>
      ...
      {message && <div style={{ ...card, position: "fixed", right: 18, bottom: 18, maxWidth: 420 }}>{message}</div>}
    </main>
  );
}
